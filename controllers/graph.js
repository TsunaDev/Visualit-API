...
module.exports = {
  ...
  */
  ...
  listBed: (service_id, status, to_clean, callback) => {
  	...
  	if (!to_clean) {
  		if (!service_id) {
	  		return GraphCallTransformInteger('MATCH (b:Bed {status: ' + status + '})-[SERVICE]->(s:Service) RETURN collect(b {.*, service_id:ID(s)})', callback)
  		} else {
  			return GraphCallTransformInteger('MATCH (b:Bed {status: ' + status + '})-[SERVICE]->(s:Service) WHERE ID(s) = ' + service_id + ' RETURN collect(b {.*, service_id:ID(s)})', callback)
  		}
  	}
	if (!service_id) {
		return GraphCallTransformInteger('MATCH (b:Bed {status: ' + status + ', to_clean:' + to_clean + '})-[SERVICE]->(s:Service) RETURN collect(b {.*, service_id:ID(s)})', callback)
  	} else {
  		return GraphCallTransformInteger('MATCH (b:Bed {status: ' + status + ', to_clean:' + to_clean + '})-[SERVICE]->(s:Service) WHERE ID(s) = ' + service_id + ' RETURN collect(b {.*, service_id:ID(s)})', callback)
  	}
  	return GraphCallTransformInteger('MATCH (b:Bed {status: ' + status + ', to_clean:' + to_clean + '})-[SERVICE]->(s:Service) WHERE ID(s) = ' + service_id + ' RETURN collect(b {.*, service_id:ID(s)})', callback)
  },
